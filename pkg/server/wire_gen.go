...
import (
	...
	encryption2 "github.com/grafana/grafana/pkg/registry/apis/secret/encryption"
	...
	encryption3 "github.com/grafana/grafana/pkg/services/encryption"
...
func Initialize(cfg *setting.Cfg, opts Options, apiOpts api.ServerOptions) (*Server, error) {
	...
	config := worker.ProvideWorkerConfig()
	databaseDatabase := database5.ProvideDatabase(sqlStore, tracer)
	outboxQueue := metadata.ProvideOutboxQueue(databaseDatabase, tracer, registerer)
	...
	providerMap := encryption2.ProvideThirdPartyProviderMap()
	encryptionManager, err := manager4.ProvideEncryptionManager(tracer, dataKeyStorage, cfg, usageStats, providerMap)
	if err != nil {
		return nil, err
	}
	...
	workerWorker, err := worker.NewWorker(config, tracer, databaseDatabase, outboxQueue, secureValueMetadataStorage, keeperMetadataStorage, ossKeeperService, encryptionManager, featureToggles, registerer)
	if err != nil {
		return nil, err
	}
	...
	v3 := apiregistry.MergeProvisioningExtras(webhookExtraBuilder)
	apiBuilder, err := provisioning2.RegisterAPIService(cfg, featureToggles, apiserverService, registerer, resourceClient, eventualRestConfigProvider, factory, accessClient, legacyMigrator, dualwriteService, usageStats, secretsService, v3)
	if err != nil {
		return nil, err
	}
